...
class PostService:
    ...
    - Business logic for Post domain
    """
    ...
    def _check_permission(token, operation):
        ...
        Example RBAC implementation:
            if operation == 'create':
                # Post requires staff or admin role
                if not any(role in token.get('roles', []) for role in ['staff', 'admin']):
                    raise HTTPForbidden("Staff or admin role required to create post documents")
            elif operation == 'read':
    ...
    @staticmethod
    def create_post(data, token, breadcrumb):
        """
        ...
        Returns:
            str: The ID of the postd post document
        """
        try:
            PostService._check_permission(token, 'create')
            ...
            post_id = mongo.create_document(config.POST_COLLECTION_NAME, data)
            logger.info(f"Created post { post_id} for user {token.get('user_id')}")
            return post_id
            ...
            raise HTTPInternalServerError(f"Failed to create post: {error_msg}")
